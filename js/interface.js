...
function onActionChange() {
  ...
  if (!FlipletLinkUtils.isEmpty(files.selectedFiles) || (selectedAction === 'document' && fileType !== 'application') || (selectedAction === 'video' && fileType !== 'video')) {
    clearUploadedFiles();
  }
...
function save(notifyComplete) {
  ...
  if (data.action === 'app' && appAction) {
    ...
    if (data.app === 'gmail.compose') {
      ...
      data.appData.to = FlipletLinkUtils.find(emailProviderData.to, function(o) { return o.type === 'to'; }) || '';
      data.appData.cc = FlipletLinkUtils.find(emailProviderData.to, function(o) { return o.type === 'cc'; }) || '';
      data.appData.bcc = FlipletLinkUtils.find(emailProviderData.to, function(o) { return o.type === 'bcc'; }) || '';
  ...
  if (['document', 'video'].indexOf(data.action) !== -1) {
    if (files.toRemove) {
      ...
      data.files = FlipletLinkUtils.isEmpty(files.selectedFiles) ? files : files.selectedFiles;
    }
  }
...
Fliplet.Pages.get()
  .then(function(pages) {
    ...
    (pages || []).forEach(function(page) {
      var pageIsOmitted = FlipletLinkUtils.some(widgetInstanceData.omitPages || [], function(omittedPage) {
